...
use cmds::cloud::{aws_cmd, container, curl_cmd, psql_cmd, sf_cmd, wget_cmd};
...
enum Commands {
    ...
    /// Salesforce CLI with compact output (org list dedup, deploy progress stripping)
    #[command(disable_help_flag = true)]
    Sf {
        /// sf arguments
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
...
fn run_cli() -> Result<i32> {
    ...
    let code = match cli.command {
        ...
        Commands::Sf { args } => sf_cmd::run(&args, cli.verbose)?,
